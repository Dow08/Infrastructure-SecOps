...
# ─── Credentials base de données — lus depuis .env ───────────────────────────
# Les valeurs par défaut maintiennent la compatibilité si .env absent.
# En production, TOUJOURS surcharger via .env (ne jamais commit les vraies valeurs).

PG_ARTICLE_NAME     = config('PG_ARTICLE_NAME',     default='meetvoice')
PG_ARTICLE_USER     = config('PG_ARTICLE_USER',     default='meetvoice')
PG_ARTICLE_PASSWORD = config('PG_ARTICLE_PASSWORD', default='meetVoice123')

MONGO_GW_USER       = config('MONGO_GW_USER',       default='gateway_user')
MONGO_GW_PASSWORD   = config('MONGO_GW_PASSWORD',   default='gateway_password_2025')
MONGO_API_USER      = config('MONGO_API_USER',       default='admin')
MONGO_API_PASSWORD  = config('MONGO_API_PASSWORD',  default='Ilaaiua18aa45')
MONGO_EVENT_USER    = config('MONGO_EVENT_USER',     default='admin')
MONGO_EVENT_PASSWORD= config('MONGO_EVENT_PASSWORD',default='AdminPass123!')
...
DATABASES = {
    ...
    'articles': {
        'ENGINE': 'django.db.backends.postgresql',
        'NAME': PG_ARTICLE_NAME,
        'USER': PG_ARTICLE_USER,
        'PASSWORD': PG_ARTICLE_PASSWORD,
        'HOST': '127.0.0.1',
...
MONGO_DATABASES = {
    'gateway':  f'mongodb://{MONGO_GW_USER}:{MONGO_GW_PASSWORD}@127.0.0.1:{MONGO_GATEWAY_PORT}/meetvoice_gateway?authSource=admin',
    'api':      f'mongodb://{MONGO_API_USER}:{MONGO_API_PASSWORD}@127.0.0.1:{MONGO_API_PORT}/meetvoice_api?authSource=admin',
    'gateway2': f'mongodb://{MONGO_API_USER}:{MONGO_API_PASSWORD}@127.0.0.1:{MONGO_API_PORT}/meetvoice_gateway?authSource=admin',
    'social':   f'mongodb://{MONGO_API_USER}:{MONGO_API_PASSWORD}@127.0.0.1:{MONGO_API_PORT}/meetvoice_social?authSource=admin',
    'event':    f'mongodb://{MONGO_EVENT_USER}:{MONGO_EVENT_PASSWORD}@127.0.0.1:{MONGO_EVENT_PORT}/admin?authSource=admin',
}
